      ...
      <Container>
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h1 className="mt-5 text-balance text-[clamp(2.1rem,5vw,3.4rem)] font-semibold">
            {c.titleA}<span className="gradient-text">{c.titleAccent}</span>
          </h1>
            ...
            <div className="card card-accent p-6 sm:p-8">
              {status === "success" ? (
                <div className="flex flex-col items-center py-10 text-center">
                  <span className="flex h-14 w-14 items-center justify-center rounded-full text-white" style={{ background: "var(--aurora)" }}>
                ...
                <form onSubmit={submit} className="space-y-4" noValidate>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="block">
                      <span className="mono-label">{f.name} <span className="text-accent">·{f.required}</span></span>
                      <input className={`mt-1.5 ${inputCls}`} value={form.name} onChange={set("name")} placeholder={f.namePh} required />
                    </label>
                    <label className="block">
                      <span className="mono-label">{f.email} <span className="text-accent">·{f.required}</span></span>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <label className="block">
                      ...
                      <select className={`mt-1.5 ${inputCls}`} value={form.interest} onChange={set("interest")}>
                        <option value="">—</option>
                        {f.interestOptions.map((o) => (<option key={o} value={o}>{o}</option>))}
                      </select>
                    </label>
                  </div>
                  <label className="block">
