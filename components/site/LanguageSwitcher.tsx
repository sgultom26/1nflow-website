...
export default function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <div
      className={`inline-flex items-center rounded-[6px] border border-[var(--hair-strong)] p-0.5 ${className ?? ""}`}
      role="group"
      aria-label="Language"
    >
      {OPTIONS.map((o) => {
        ...
        return (
          <button
            ...
            onClick={() => go(o.code)}
            aria-pressed={active}
            className={`rounded-[4px] px-2 py-1 font-mono text-[11px] tracking-wide transition-colors ${
              active ? "bg-[var(--color-accent)] text-white" : "text-muted hover:text-fg"
            }`}
          >
            {o.label}
          </button>
